...
func (ng *Engine) findMinTime(s *parser.EvalStmt) time.Time {
	...
	parser.Inspect(context.TODO(), s.Expr, func(node parser.Node, path []parser.Node) error {
...
func (ng *Engine) populateSeries(querier storage.Querier, s *parser.EvalStmt) {
	...
	n, err := parser.Inspect(context.TODO(), s.Expr, func(node parser.Node, path []parser.Node) error {
		l.Lock()
		defer l.Unlock()
		switch n := node.(type) {
		case *parser.VectorSelector:
