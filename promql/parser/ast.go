...
func Walk(ctx context.Context, v Visitor, node Node, path []Node, nr NodeReplacer) (Node, error) {
	...
	for i, e := range Children(node) {
		if childNode, err := Walk(ctx, v, e, path, nr); err != nil {
			return node, err
		} else {
...
func Inspect(ctx context.Context, node Node, f inspector, nr NodeReplacer) (Node, error) {
	//nolint: errcheck
	return Walk(ctx, inspector(f), node, nil, nr)
}
...
type NodeReplacer func(context.Context, Node) (Node, error)
